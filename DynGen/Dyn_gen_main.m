...
for i = 1:numJ
    ...
    joint_pos_extend = T*[0;0;1];
    joint_pos(:,i) = joint_pos_extend(1:2);
    w = w+dq(i);
    v_extend = simplify([-1,0,0;0,1,0;0,0,1]*w*T*[rc_e(:,i);1]);
    vc_e(:,i) = v_extend(1:2,1);
    p_e(:,i) = m_e(i)*vc_e(:,i);
end
all_tasks{1,task_i} = @()matlabFunction(p_e,'file','exo_momentum','vars',{q,dq});
task_i=task_i+1;
all_tasks{1,task_i} = @()matlabFunction(rcm_pos,'file','exo_com_pos','vars',{q});
task_i = task_i+1;
all_tasks{1,task_i} = @()matlabFunction(joint_pos,'file','exo_joint_pos','vars',{q});
task_i = task_i+1;
...
all_tasks{1,task_i} = @()matlabFunction(p_h,'file','human_no_load_momentum','vars',{q,dq});
task_i=task_i+1;   
all_tasks{1,task_i} = @()matlabFunction(rcm_pos,'file','human_no_load_com_pos','vars',{q});
task_i = task_i+1;   
all_tasks{1,task_i} = @()matlabFunction(joint_pos,'file','human_joint_pos','vars',{q});
task_i = task_i+1;
%% human with load
...
all_tasks{1,task_i} = @()matlabFunction(p_h,'file','human_load_momentum','vars',{q,dq});
task_i=task_i+1;   
